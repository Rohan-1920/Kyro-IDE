import { exportAllSettings } from "@/lib/server/services/settings-service";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const rate = checkRateLimit(`settings:export:${userId}`, 30, 60_000);
    ...
    return ok(await exportAllSettings(projectId, userId), 200, "SETTINGS_EXPORTED", requestId);
  } catch (err) {
    return handleRouteError(err, { route: "/api/settings/export", requestId });
  }
}
